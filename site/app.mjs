...
const wireInput = (session, fail) => {
  ...
  wired = {
    ...
    // The OTHER input channel. xterm splits what it sends us in two:
    // onData carries text (a JS string meant to be UTF-8 encoded),
    // onBinary carries byte sequences that are NOT text -- mouse
    // reports in the legacy X10/URXVT encodings, whose coordinate
    // bytes (32 + coordinate) run past 0x7f the moment the pointer is
    // past column ~95. Those arrive as a latin1-in-a-JS-string, one
    // char per byte, and must go on the wire byte for byte: running
    // them through TextEncoder would UTF-8-expand every high byte into
    // two and hand the remote app coordinates that do not exist. That
    // segregation is the whole reason xterm has a second event.
    //
    // Unwired, this was not a corner case: vim (not neovim) asks the
    // terminal's Secondary DA for a version before it will use SGR
    // mouse reports, xterm.js answers 276 and vim wants >= 277, so it
    // settles on the legacy encoding -- and every click past column
    // ~95 in a desktop vim vanished on the way to the host.
    //
    // Deliberately NOT routed through transformInput: these chunks are
    // generated by the terminal, never typed, so a sticky Ctrl has no
    // business landing on one (mobile.mjs makes the same argument from
    // the other side for the reports that ride onData).
    binary: term.onBinary((s) => {
      if (currentSession !== session || window.__wosh.failure) return;
      session
        .writeInput(Uint8Array.from(s, (c) => c.charCodeAt(0) & 0xff))
        .then(wakeNow, fail);
    }),
  };
};
