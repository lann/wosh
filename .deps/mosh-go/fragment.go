...
const (
	...
	// Maximum payload per fragment. Upstream uses 1300 to stay under
	// typical MTUs; we use 1100 so the worst-case wire datagram
	// (8B nonce + 4B timestamps + 10B fragment header + payload +
	// 16B OCB tag = payload + 38) stays under the ~1162 B iroh QUIC
	// datagram ceiling (1200 B MTU profile). Fragment sizing is a
	// sender-local choice in SSP; receivers reassemble by (id, num).
	maxFragmentPayload = 1100
)
